        ...
        <nav className="flex-1 overflow-y-auto py-1 px-2">
          ...
          {drawerNavItems.map(({ label, path, icon: Icon }) => {
            ...
            return (
              <NavLink key={label} to={profilePath} onClick={() => onOpenChange(false)}
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-lg px-3 py-2.5 text-[16px] font-semibold transition-colors ${isActive ? "text-foreground bg-accent" : "text-foreground hover:bg-accent"}`
                }>
          ...
          {isStaff && (
            <NavLink to="/admin" onClick={() => onOpenChange(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-2.5 text-[16px] font-semibold transition-colors ${isActive ? "text-primary bg-primary/10" : "text-primary hover:bg-primary/5"}`
              }>
