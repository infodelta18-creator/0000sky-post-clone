...
export default function HashtagPage() {
  ...
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("posts").select("id, content, created_at, parent_id, author_id, profiles!posts_author_id_fkey(id, username, display_name, avatar_url)").ilike("content", `%#${tag}%`).is("parent_id", null).order("created_at", { ascending: false }).limit(50);
      ...
      return data.map((p: any) => {
        ...
        return { id: p.id, authorId: p.author_id, authorName: profile?.display_name || "", authorHandle: profile?.username || "", authorAvatar: profile?.avatar_url || "", content: p.content, createdAt: p.created_at, images: postImages[p.id], likeCount: likeCounts[p.id] || 0, replyCount: replyCounts[p.id] || 0, repostCount: repostCounts[p.id] || 0, isLiked: userLikedSet.has(p.id), isReposted: userRepostedSet.has(p.id) };
      });
    },
    enabled: !!tag,
  });
