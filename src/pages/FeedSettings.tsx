  ...
  return (
    <div className="flex flex-col">
      <div className="sticky top-0 z-20 flex items-center justify-between border-b border-border bg-background/95 px-4 py-3 backdrop-blur-sm">
        <div className="flex items-center gap-2"><button onClick={() => navigate("/feeds")} className="p-1"><ArrowLeft className="h-5 w-5 text-foreground" /></button><h2 className="text-lg font-bold">{t("nav.feeds")}</h2></div>
        <Button variant="outline" size="sm" className="rounded-full" onClick={() => navigate("/feeds")}><Save className="h-4 w-4 mr-1" />{t("feed_settings.save")}</Button>
      </div>
      <div className="px-4 pt-5 pb-2"><h3 className="text-xl font-bold text-foreground">{t("feeds.pinned")}</h3></div>
        ...
        {pinnedList.map((feed: any) => {
          ...
          return (
            <div key={feed.id + (feed._ufId || "")} className="flex items-center gap-3 px-4 py-3.5 border-b border-border">
              ...
              <div className="flex-1 min-w-0"><p className="font-semibold text-[15px] text-foreground truncate">{feed.name}</p><p className="text-sm text-muted-foreground truncate">{t("feeds.feed_by")} {feed.author_handle}</p></div>
              <div className="flex items-center gap-1">
                {isUserPinned && (<>
                  <button onClick={() => movePinned.mutate({ ufId: feed._ufId, direction: "up" })} className="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-foreground"><ArrowUp className="h-4 w-4" /></button>
                  <button onClick={() => movePinned.mutate({ ufId: feed._ufId, direction: "down" })} className="p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-foreground"><ArrowDown className="h-4 w-4" /></button>
                </>)}
      ...
      {savedList.length > 0 && (<>
        <div className="px-4 pt-6 pb-2"><h3 className="text-xl font-bold text-foreground">{t("feeds.saved")}</h3></div>
        <div className="border-t border-border">
      ...
      <div className="px-4 py-6"><p className="text-sm text-muted-foreground">{t("feeds.guide")} <a href="#" className="text-primary hover:underline">{t("feeds.see_guide")}</a></p></div>
    </div>
  );
}
