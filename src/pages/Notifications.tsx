...
export default function Notifications() {
  ...
  useEffect(() => {
    ...
    supabase.from("notifications").update({ read: true }).eq("user_id", user.id).eq("read", false)
      .then(() => { queryClient.invalidateQueries({ queryKey: ["unread_notifications_count"] }); });
  }, [notifications, user]);
  ...
  const followMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["my_following_ids"] }); },
  });
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="sticky top-0 z-20 border-b border-border bg-background/95 backdrop-blur-sm">
        ...
        <div className="flex">
          <TabButton label={t("notif.all")} active={tab === "all"} onClick={() => setTab("all")} />
          <TabButton label={t("notif.mentions")} active={tab === "mentions"} onClick={() => setTab("mentions")} />
        </div>
      </div>

      {isLoading ? (
        <div>{Array.from({ length: 5 }).map((_, i) => <NotificationSkeleton key={i} />)}</div>
      ) : grouped.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 py-12">
          <Bell className="h-12 w-12 text-muted-foreground" strokeWidth={1.5} />
          <p className="text-muted-foreground">{t("notif.no_notifications")}</p>
        </div>
      ) : (
        <div>
          {grouped.map((item, idx) => {
            if (item.actors.length > 1) {
              return <GroupedNotification key={idx} item={item} navigate={navigate} myFollowing={myFollowing} onFollow={(id) => followMutation.mutate(id)} typeText={typeText} t={t} />;
            }
...
function groupNotifications(notifications: any[]) {
  ...
  notifications.forEach((n: any) => {
    ...
    if (last && last.type === n.type && n.type === "follow" && last.notifications.length < 5) {
      last.actors.push(n.actor); last.notifications.push(n);
    } else {
      result.push({ type: n.type, actors: [n.actor], notifications: [n], postId: n.post_id });
    }
  });
  return result;
}

function GroupedNotification({ item, navigate, myFollowing, onFollow, typeText, t }: any) {
  ...
  return (
    <div className={`flex gap-3 px-4 py-3.5 border-b border-border cursor-pointer transition-colors hover:bg-accent/30 ${isUnread ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}
      onClick={() => item.postId ? navigate(`/post/${item.postId}`) : navigate(`/profile/${firstActor?.username}`)}>
      <div className={`mt-1 flex-shrink-0 ${typeColors[item.type]}`}><Icon className="h-5 w-5" strokeWidth={1.75} fill={item.type === "like" ? "currentColor" : "none"} /></div>
      <div className="min-w-0 flex-1">
        <div className="flex -space-x-2 mb-1.5">
          {item.actors.slice(0, 3).map((actor: any, i: number) => (
            <Avatar key={i} className="h-9 w-9 border-2 border-background"><AvatarImage src={actor?.avatar_url} /><AvatarFallback className="bg-primary text-primary-foreground text-xs">{actor?.display_name?.[0]?.toUpperCase()}</AvatarFallback></Avatar>
          ))}
          {item.actors.length > 3 && <div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted border-2 border-background text-xs font-medium text-muted-foreground">+{item.actors.length - 3}</div>}
        </div>
        <p className="text-[15px]">
          <strong className="hover:underline cursor-pointer" onClick={(e: any) => { e.stopPropagation(); navigate(`/profile/${firstActor?.username}`); }}>{firstActor?.display_name}</strong>
          {othersCount > 0 && <> {t("notif.and")} <strong>{othersCount} {othersCount === 1 ? t("notif.other") : t("notif.others")}</strong></>}{" "}
          <span className="text-muted-foreground">{typeText[item.type] || item.type} · {timeAgo(item.notifications[0].created_at)}</span>
        </p>
        {item.type === "follow" && (
          <div className="flex gap-2 mt-2 flex-wrap">
            {item.actors.filter((a: any) => !myFollowing.includes(a?.id)).slice(0, 2).map((actor: any) => (
              <button key={actor?.id} onClick={(e: any) => { e.stopPropagation(); onFollow(actor.id); }}
...
const typeIcons2 = typeIcons;
const typeColors2 = typeColors;

function TabButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`relative flex-1 py-3 text-sm font-semibold transition-colors ${active ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
      {label}
